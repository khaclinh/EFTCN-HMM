...
class Trainabble_Multiply(Layer): # a scaled layer
    def __init__(self, **kwargs):
        super(Trainabble_Multiply, self).__init__(**kwargs)
    def build(self, input_shape):
        self.output_dim = input_shape[1] 
        self.W = self.add_weight(shape=(1,), # Create a trainable weight variable for this layer.
                                 initializer='one', trainable=True)
        super(Trainabble_Multiply, self).build(input_shape)  # Be sure to call this somewhere!
...
class ED_TCN_fmap_fuse():
    ...
    def create_model(self):
        ...
        x = layers.MaxPool1D(self.before_encode_pool_size)(hand_acc)
