...
def preprocess(targetdir, outdatapath, columns_to_use, exclude_activities, fold,
    ...
    frame_length = int(2.56 * 100)
    step = 1 * 100
    if not fold:
        if val_test_size is None:
            
            
            for index in range(len(datasets_filled)):
                X, Y = split_data(Xlists, ybinarylists, indices=index)
                x_sample, y_sample = sliding_window(frame_length, step, X, Y)
                
                numpify_and_store(x_sample, y_sample, X_name='x_data' + str(index), y_name='y_data' + str(index),
...
def load_data(outputpath, numOfSubjects):
    ...
    for index in range(numOfSubjects):
        x = np.load(os.path.join(outputpath, 'x_data' + str(index) + ext))
        y = np.load(os.path.join(outputpath, 'y_data' + str(index) + ext))
